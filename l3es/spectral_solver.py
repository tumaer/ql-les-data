...
def set_up_solver(
    N, nu, dim, case, dt, seed, kf, e_kin_target=1.0, forcing_type="ekin_tot"
):
    L = 2 * jnp.pi
    dx = L / N
...
def simulate(
    ...
    u, u_hat, xyz_vis, dx, integrate_fn, _, _, e_kin_init, e_inj = set_up_solver(
        N, nu, dim, case, dt, seed, kf, e_kin_target, forcing_type
    )
    ...
    if case == "HIT" and forcing_type != "none":
        hit_eddy_turnover_time = e_inj_acc / tstep
        ...
        print(f"Average energy injected =       {e_inj_acc / tstep:.3f}")

        kmax = jnp.sqrt(2) * N / 3
        kolm_scale = (nu**3 / (e_inj_acc / tstep)) ** (0.25)
        print(f"kmax*eta =                      {kmax*kolm_scale:.3f}")

        Re_lambda = jnp.sqrt(20 * e_kin**2 / (3 * nu * e_inj_acc / tstep))
