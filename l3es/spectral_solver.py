...
def rk4_wrapper(dt, rhs_fn, forcing_mask, e_kin_init, forcing_type, fft_axes=(1, 2, 3)):
    ...
    def step_fn(u, u_hat):
        ...
        e_inj = 0.0
        if forcing_mask is not None and forcing_type == "ekin_tot":
...
def simulate(
    ...
    Args:
        case (str): Simulation case. One of ["TGV3D", "HIT", "Kolm", ...].
        N (int): Grid size. Nx=Ny(=Nz)=N.
        dim (int): Dimension.
        nu (float): Viscosity = 1/Re.
        ...
        forcing_type (str): Forcing type for HIT case. One of ["ekin_tot", "none"].
            ekin_tot: rescale to keep total kinetic energy constant; none: no forcing.
    ...
    for i in range(tstep_max + 1):
        ...
        if i % log_freq == 0:
            ...
            print(
                f"Step {i}/{tstep_max}, u_max = {umax:.3f}, E_kin = {e_kin:.3f}, "
